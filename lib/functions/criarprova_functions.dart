...
class CriarProvaFunctions {
  ...
  bool todosCamposPreenchidos({
    ...
    required curso_model.Curso? cursoSelecionado,
    required Map<String, dynamic>? materiaSelecionada,
    required Map<String, dynamic>? professorSelecionado,
  }) {
    return dia != null &&
  ...
  // Função para salvar prova (suporta múltiplos cursos)
  Future<void> salvarProva({
    required DateTime? dia,
    required String? periodoAulaSelecionado,
    required sala_model.Sala? salaSelecionada,
    ...
    // Para cada curso, verifica limites e conflitos
    for (final curso in cursosSelecionados) {
      ...
      // 2. Verifica quantos agendamentos do tipo Prova já existem para a sala nesse dia, período e aula_periodo específico
      ...
      if (conflitosHorario.isNotEmpty) {
        ...
        if (tiposExistentes.contains('M')) {
          throw Exception('Já existe uma prova agendada para este horário.');
        }
    ...
    // Salva agendamentos para todos os cursos selecionados
    for (final curso in cursosSelecionados) {
      final periodoCurso = curso.periodo;
      ...
      try {
        ...
        throw Exception(
          'Erro ao salvar agendamento para o curso ${curso.curso}: $e',
        );
      }
    }
